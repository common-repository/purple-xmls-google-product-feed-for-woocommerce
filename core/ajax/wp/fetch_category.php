...
if ( class_exists( 'EXCPF_Taxonomy' ) ) {
	$data = EXCPF_Taxonomy::onLoadTaxonomy( strtolower( $service_name ) );
}
...
foreach ( $data as $this_item ) {
	...
	if ( strpos( strtolower( $this_item ), $searchTerm ) !== false ) {
		...
		if ( $canDisplay ) {
			echo '<div class="categoryItem" onclick="doSelectCategory_default(this, \'' . esc_html( $option ) . '\', \'' . esc_html( $service_name ) . '\')">' . esc_html( $text ) . '</div>';
		}
